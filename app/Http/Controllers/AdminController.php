...
class AdminController extends BaseController
{
	...
	function ManageUser($IsActive)
	{
		if(Session::get('LoginData')['Role']==1)
		{
			...
			{
				$MainData['Users']=GetUserData::where('IsActive','1')->orderBy('CreatedDate', 'desc')->get();
			}
			if($IsActive=="DeactiveUser")
			{
				$MainData['Users']=GetUserData::where('IsActive','0')->orderBy('CreatedDate', 'desc')->get();
			}
	...
	function ActiveDeactiveUser(Request $request)
	{
		...
		if(Session::get('LoginData')['Role']==1)
		{
			...
			{	
				$this->ActiveDeactiveUserInPHMeter($data);
	...
	function SaveDevice(Request $request)
	{
		...
		if(Session::get('LoginData')['Role']==1 || Session::get('LoginData')['Role']==4)
		{
			...
			$Master_Calibration_Date=$request['Master_Calibration_Date'];

			$data=[];
			$data['Master_Calibration_Date']=$Master_Calibration_Date;
...
// print_r($data);die;
	...
	function SaveDeviceWeight(Request $request)
	{
		...
		if(Session::get('LoginData')['Role']==1 || Session::get('LoginData')['Role']==4)
		{
			...
			if(isset($request['DPWeight']) && isset($request['DLineId1']))
			{
				...
				foreach ($request['DLineId1'] as $key => $val)
				{
					if($request['DPWeight'][$key]!="")
					{
						...
						$LineData3['ActualMass']=$request['ActualMass'][$key];
						$LineData3['Type']=0;
			...
			if($request['LStWeight'][$key]!="" && $request['LCertWeight'][$key]!="" && $request['LAccpWeight'][$key]!="")
				{
			$GetCalibrationLine = new GetDeviceWeight();

			$CalibrationLine=[];
			foreach ($request['LLineId'] as $key => $val)
			{
				if($request['LStWeight'][$key]!="" && $request['LCertWeight'][$key]!="" && $request['LAccpWeight'][$key]!="")
				{
					$LineData4['InstrumentId']=$InstrumentID;
					$LineData4['DeviceId']=$DeviceId;
					$LineData4['CalibrationTypeId']=$CalibrationFormType;
					$LineData4['Type']=3;
					$LineData4['LineId']=$request['LLineId'][$key];
					$LineData4['StWeight']=$request['LStWeight'][$key];
					$LineData4['CertWeight']=$request['LCertWeight'][$key];
					$LineData4['AccpWeight']=$request['LAccpWeight'][$key];
					$LineData4['CreatedBy']=Session::get('LoginData')['UserId'];

					$CalibrationLine[$key]=$LineData4;
				}
			}
			$saveddata4=$GetCalibrationLine->SaveDeviceWeight($CalibrationLine);
			}
			else
			{
				$saveddata4=1;
			...
			if($saveddata && $saveddata2 && $saveddata3 && $saveddata4)
			{
	...
	function EditDeviceWeight(Request $request)
	{
		...
		$MainData['DeviceWeight']=GetDeviceWeight::where('InstrumentId',$request['InstrumentId'])->where('DeviceId',$request['DeviceId'])->where('CalibrationTypeId',$request['CalibrationTypeId'])->orderBy('LineId', 'asc')->where('Type',0)->get();
		if($MainData['DeviceWeight'][0]->CalibrationTypeId==2)
		{
			...
			// $MainData['CalibrationLine']=GetCalibrationLine::where('InstrumentId',$request['InstrumentId'])->where('DeviceId',$request['DeviceId'])->where('CalibrationTypeId',$request['CalibrationTypeId'])->orderBy('LineId', 'asc')->get();
			
			$MainData['CalibrationLine']=GetDeviceWeight::where('InstrumentId',$request['InstrumentId'])->where('DeviceId',$request['DeviceId'])->where('CalibrationTypeId',$request['CalibrationTypeId'])->where('Type',3)->orderBy('LineId', 'asc')->get();

			//print_r($MainData['CalibrationLine']);die;
		}else{
			$MainData['CalibrationLine']=[];
		}
	...
	function UpdDeviceWeight(Request $request)
	{
		...
		if(Session::get('LoginData')['Role']==1 || Session::get('LoginData')['Role']==4)
		{
			...
			$GetDeviceWeight1 = new GetDeviceWeightObsMass();
			$GetDeviceWeight2 = new GetDevicePositionWeight();
			$GetDeviceWeight3 = new GetDeviceWeight();
			...
			$DeleteDeviceData1=[];
			// $DeleteDeviceData1['InstrumentId']=$InstrumentID;
			// $DeleteDeviceData1['DeviceId']=$DeviceId;
			// $DeleteDeviceData1['CalibrationTypeId']=$CalibrationFormType;
			// $DeleteDeviceData1['Type']=3;
			//print_r($DeleteDeviceData);die;
			...
			$GetDeviceWeight3->DeleteDeviceWeightType3($DeleteDeviceData);
			...
			if(isset($request['LStWeight']) && isset($request['LCertWeight']) && isset($request['LAccpWeight']))
			{		 	
	...
	function DeleteDeviceWeight(Request $request)
	{
		if(Session::get('LoginData')['Role']==1 || Session::get('LoginData')['Role']==4)
		{
			...
			if($GetDeviceWeight->DeleteDeviceWeight($DeleteDeviceData) && $GetDeviceWeight1->DeleteDeviceWeightObsMass($DeleteDeviceData) && $GetDeviceWeight2->DeleteDevicePositionWeight($DeleteDeviceData) && $GetDeviceWeight3->DeleteDeviceWeightType3($DeleteDeviceData))
			{
	...
	function SaveLdapConfiguration(Request $request)
	{
		...
		if(Session::get('LoginData')['Role']==1)
		{
			...
			if(isset($request['chkLDAP_ADMIN']))
			{
				...
				$base2=base64_encode($base1);
				$base3=base64_encode($base2);
				$encryptedLDAP_ADMIN=base64_encode($base2);
			...
			$record = DB::table('ldapconfiguration')->get();

			$data['LDAP_HOSTNAME']=isset($LDAP_HOSTNAME)?$LDAP_HOSTNAME:$record[0]->LDAP_HOSTNAME;
			$data['LDAP_ADMIN']=isset($encryptedLDAP_ADMIN)?$encryptedLDAP_ADMIN:$record[0]->LDAP_ADMIN;
			$data['LDAP_PASSWORD']=isset($base3)?$base3:$record[0]->LDAP_PASSWORD;
			$data['LDAP_DC1']=isset($LDAP_DC1)?$LDAP_DC1:$record[0]->LDAP_DC1;;
			$data['LDAP_DC2']=isset($LDAP_DC2)?$LDAP_DC2:$record[0]->LDAP_DC2;;

			DB::table('ldapconfiguration')->where('RecId', $record[0]->RecId)->update($data);

			$this->UpdateLdapConfigurationInPHMeter($data);
	...
	// end rhl 02-12-2020 //
	function ManageFormulaSettings()
	{
		//if(Session::get('LoginData')['Role']==1)
		if(Session::get('LoginData')['Role']==1 || Session::get('LoginData')['Role']==4)
		{
			$MainData = DB::table('formula_setting')->get();
			
			return view('ManageFormulaSettings',['MainData'=>$MainData[0]]);
	...
	function SaveFormulaSettings(Request $request)
	{
		//if(Session::get('LoginData')['Role']==1)
		if(Session::get('LoginData')['Role']==1 || Session::get('LoginData')['Role']==4)
		{

			$data['monthlytype']=$request['monthlytype'];
			$data['MonthlyDisplayValue']=$request['MonthlyDisplayValue'];
			$data['microtype']=$request['microtype'];
			$data['microDisplayValue']=$request['microDisplayValue'];

			$saveddata = '';
			$record = DB::table('formula_setting')->get();
		
			if(isset($record[0]->RecId)){
			 	$saveddata = DB::table('formula_setting')->where('RecId', $record[0]->RecId)->update($data);
			}else{
			 	$saveddata=DB::table('formula_setting')->insert($data);
			}
			
			if($saveddata)
			{
				return redirect("/ManageFormulaSettings")->with('success', 'Formula Settings has been updated successfully.');
			}
			else
			{
				return redirect("/ManageFormulaSettings")->with('failed', 'Unable to update Formula settings...');
			}
		}
		else
		{
	...
	/*API Code Start 14.07.2021*/

	function UpdUser1InPHMeter($data)
	{
		//print_r($data);die;
		//echo 'TT';die;
		$url = 'http://10.21.3.51:81/api/getUpdUser1InPHMeter/';
...
// Close cURL resource
	...
	function ActiveDeactiveUserInPHMeter($data)
	{
		$url = 'http://10.21.3.51:81/api/getActiveDeactiveUserInPHMeter/';
...
// Close cURL resource
	...
	/*API Code End 14.07.2021*/

/*API Code Start 19.07.2021*/
	function UpdateLdapConfigurationInPHMeter($data)
	{
		//$url = 'http://10.21.3.51:81/api/getUpdateLdapConfigurationInPHMeter/';
$url = 'http://localhost/phmeter/api/getUpdateLdapConfigurationInPHMeter/';
// Create a new cURL resource
		$ch = curl_init($url);
