...
class LdapController extends BaseController
{
	...
	function SaveUser(Request $request)
	{
		if(Session::get('LoginData')['Role']==1)
		{
			...
			if($UserDataC<1)
			{
				...
				{
					$LDAPUsers = DB::table('ldapconfiguration')->get();
		
					$hostname=$LDAPUsers[0]->LDAP_HOSTNAME;
					$username=$this->SecureKeyToPassword($LDAPUsers[0]->LDAP_ADMIN);
					$ldap_dn = $username."@".$hostname;
					$ldap_password = $this->SecureKeyToPassword($LDAPUsers[0]->LDAP_PASSWORD);
					...
					if(@ldap_bind($ldap_con, $ldap_dn, $ldap_password)) 
					{
					  ...
					  //$filter="(sAMAccountName=$UserId)";
						$result = @ldap_search($ldap_con,"dc=".$LDAPUsers[0]->LDAP_DC1.",dc=".$LDAPUsers[0]->LDAP_DC2."",$filter) or exit("Unable to search");
	...
	function getSaveUserInCalibration()
	{
		$data = json_decode(file_get_contents('php://input'), true);
		
		$GetUserData = new GetUserData();
		$GetUserData->SaveUser($data);
	}

	function getUpdateUserProfileInCalibration()
	{
		$data = json_decode(file_get_contents('php://input'), true);
	...
	function getUpdUser1InCalibration()
	{
		$data = json_decode(file_get_contents('php://input'), true);
	...
	function getActiveDeactiveUserInCalibration()
	{
		$data = json_decode(file_get_contents('php://input'), true);
		
		$GetUserData = new GetUserData();
		$GetUserData->ActiveDeactiveUser($data);
	}

	/*13.07.2021 Start*/

	function SaveUserInPHMeter($data)
	{

		$url = 'http://10.21.3.51:81/api/getSaveUserInPHMeter/';
...
// Close cURL resource
  ...
  //       }die;
	...
	function UpdateUserProfileInPHMeter($data)
	{
		...
		$url = 'http://10.21.3.51:81/api/getSaveUserProfileInPHMeter/';
...
// Close cURL resource
	...
	/* LdapConfiguration API 19.07.2021 */
	function getUpdateLdapConfigurationInCalibration()
	{
		$data = json_decode(file_get_contents('php://input'), true);
