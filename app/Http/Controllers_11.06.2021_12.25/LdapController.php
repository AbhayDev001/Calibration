	...
	function UserLogin(Request $request)
	{
		...
		if($UserDataC>0)
		{
			...
			if($UserData['IsActive']==1)
			{
				if($UserData['LdapUser']==0)
				{
					...
					else
					{
						...
						$Logindata['Status']=0;
						$LoginLog->SaveLoginLog($Logindata);
						return redirect("/Login")->with('failed', 'Login failed ! UserId or Password incorrect');
					}
				}
				else
				{
					...
					else
					{
						...
						$Logindata['Status']=0;
						$LoginLog->SaveLoginLog($Logindata);
						return redirect("/Login")->with('failed', 'Login failed ! UserId or Password not found on server');
					}
				}
			}
			else
			{
				...
				$Logindata['Status']=0;
				$LoginLog->SaveLoginLog($Logindata);
				return redirect("/Login")->with('failed', 'Login failed ! Your userid is deactive contact to administrator');
			}
		}
		else
		{
