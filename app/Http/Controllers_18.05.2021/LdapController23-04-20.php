		...
		if(Session::get('LoginData')['Role']==1)
		{
			...
			if($UserDataC<1)
			{
				if($LocalUser==1)
				{
					...
					{
						return redirect("/ManageUser")->with('success', 'User has been added successfully.');
					}
					else
					{
						return redirect("/ManageUser")->with('failed', 'Unable to save user...');
					}
				}
				else
				{
					...
					//$filter="(sAMAccountName=$UserId)";
						...
						{
							return redirect("/ManageUser")->with('failed', 'user not found on server...');		
						}
					}
					else
					{
						return redirect("/ManageUser")->with('failed', 'Unable to get admin credentials please contact to administrator');
					}	
				}
			}
			else
			{
				return redirect("/ManageUser")->with('failed', 'UserId already exists in database...');
			}
		}
		else
		{
