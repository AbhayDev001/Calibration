...
class GetDeviceWeight extends Model
{
	...
	public function DeleteDeviceWeightType3($data)
	{
		$this->where('DeviceId', $data['DeviceId'])->where('InstrumentId', $data['InstrumentId'])->where('CalibrationTypeId', $data['CalibrationTypeId'])->where('Type', 3)->delete();
		return 1;
	}
