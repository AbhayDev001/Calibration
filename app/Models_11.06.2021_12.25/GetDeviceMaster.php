...
class GetDeviceMaster extends Model
{
	...
	protected $fillable=['Name','InstrumentId','Make','Model','SearialNo','IsActive','DirPath','DeviceType','CreatedBy','Instrument_No'];
	...
	{
		$this->where('RecId', $data['RecId'])->update(array('Name' => $data['Name'],'InstrumentId' => $data['InstrumentId'],'Make' => $data['Make'],'Model' => $data['Model'],'SearialNo' => $data['SearialNo'],'DirPath' => $data['DirPath'],'DeviceType' => $data['DeviceType'],'Instrument_No'=>$data['Instrument_No']));
		return 1;
	}
