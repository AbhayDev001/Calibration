								...
								<table class="table table-striped table-bordered table-hover" width="100%">
									...
									<tbody>
										...
										<tr>
											...
											<td>
												<input type='text' class='ActualMass TabInput numeric1' readonly="readonly" autocomplete="off" style="width: 100%;" name='ActualMass[]' value='{{ $val->ActualMass }}'/>
											</td>
											<td>
                                                ...
                                                <input type='hidden' class='ResObserverMass TabInput numeric1' readonly="readonly" autocomplete="off" style="width: 100%;" name='ResObserverMass[]' />
											</td>
											<td>
...
<script type="text/javascript">
			...
			if(DeviceFilePath!='')
			{
				$.get('../AjaxGetFileData/'+ DeviceFilePath,function(data){
					if(data['mg']!="")
					{
						...
						var ObserverMass=Number(ActualMass)-Number(data['mg']);
						ThisTr.find(".ObserverMass").val(data['mg']);
                        ThisTr.find(".ResObserverMass").val(ObserverMass.toFixed(2));
						GetVal();
					}
					else
					{
	...
	function GetVal(){
		...
		$('.ResObserverMass').each(function(){
            // alert($(this).val());
			//if(RealValue<Number($(this).val()))
             if(RealValue>Number($(this).val()) && -RealValue>Number($(this).val()))
			{
				ac=2;
			}
		});
