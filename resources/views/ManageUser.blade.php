...
<div id="content">
	...
	<section>
				...
				<div class="product-content product-wrap clearfix product-deatil">
					<div class="col-lg-12">
						<div class="row top-menu-box">
							...
							if($segment=='ActiveUser'){
							$admin_count = DB::table('usermaster')
							->where(['Role'=> 1])
							->where(['IsActive'=> 1])
							->count(); 

							$analysis_count = DB::table('usermaster')
							->where(['Role'=> 2])
							->where(['IsActive'=> 1])
							->count(); 
            

							$verify_count = DB::table('usermaster')
							->where(['Role'=> 3])
							->where(['IsActive'=> 1])
							->count(); 
            

							$approver_count = DB::table('usermaster')
							->where(['Role'=> 4])
							->where(['IsActive'=> 1])
							->count(); 
						}
							...
							<div class="col-md-2">
								<b>Total Admin:- <?php echo $admin_count;?></b>
							</div>
							<div class="col-md-2">
								<b>Total Analysis:- <?php echo $analysis_count;?></b>
							</div>
							<div class="col-md-2">
								<b>Total Verify:- <?php echo $verify_count;?></b>
							</div>
							<div class="col-md-2">
								<b>Total Approver:- <?php echo $approver_count;?></b>
							</div>
								...
								<tbody>
									...
									<tr class="@if($user->LdapUser==0) {{ 'LocalUser' }} @endif">
										...
										<td>
											...
											<!-- <a href="javascript:void(0)" data-RecId="{{ $user->UserId }}" class="btn btn-xs btn-default btnEditForm"><i class="fa fa-pencil"></i></a> -->
											...
											<form name="frm" method="post" action="{{ url('ActiveDeactiveUser') }}" style="display: inline-block;"> 
												...
												<button type="submit" class="hidden-submit-button" name="submit" style="display: none;">submit</button>
												@if($user->IsActive==0)

												<button type="button" title="Active Now" class="btn btn-xs btn-default btn-active"><i class="fa fa-check"></i></button>

												@else
												<button type="button" title="Deactive Now" class="btn btn-xs btn-default btn-inactive"><i class="fa fa-close"></i></button>
												@endif
											</form>
										</td>
									</tr>
		...
		<!-- <div class="modal fade" id="ActiveDeactiveModal" role="dialog">
			<div class="modal-dialog">
				<div class="modal-content">
					<form id="UpdUser" name="frm" method="post" action="{{ url('ActiveDeactiveUser') }}">
						<input type="hidden" value="{{csrf_token()}}" name="_token" />
						<input type="text" value="0" name="RecId" class="UpdRecId" />
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal">&times;</button>
							<h4 class="modal-title">Are you sure want to deactive user?</h4>
						</div>
...
<script type="text/javascript">
	/*$(".btnEditForm").click(function(){
		$.get('../AjaxGetUserData/'+ $(this).attr("data-RecId"),function(data){
			...
			{
			alert(data);
				$(".UpdRecId").val(data['UserId']);
				//$(".UpdRecId").val(data['is']);
	...
	$(".btn-active").click(function(){
		if(confirm('Are you sure want to active user ?')){
			$(this).parent('form').find('.hidden-submit-button').trigger('click');
		}else{
			return false;
		}
	});
	$(".btn-inactive").click(function(){
		if(confirm('Are you sure want to inactive user ?')){
			$(this).parent('form').find('.hidden-submit-button').trigger('click');
		}else{
			return false;
		}
