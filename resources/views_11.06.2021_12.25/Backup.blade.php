				...
				<div class="product-content product-wrap clearfix product-deatil">
					...
					<div class="col-lg-12 mt20">
						<form name="frm" method="post" action="{{ url('/BackupData') }}">
									...
									<div class="form-group">
										...
										<div class="row">
											<div class="col-lg-5 col-md-5 col-sm-12 col-xs-12">
												<input type="date" name="FromCreatedDate" id="FromCreatedDate" class="form-control" value='@if(isset($MainData["FromCreatedDate"])){{ $MainData["FromCreatedDate"]}}@endif' required>
											</div>
											<div class="col-lg-1 col-md-1 col-sm-12 col-xs-12">
												<b>TO</b>
											</div>
											<div class="col-lg-5 col-md-5 col-sm-12 col-xs-12">
												<input type="date" name="ToCreatedDate" id="ToCreatedDate" class="form-control" value='@if(isset($MainData["ToCreatedDate"])){{ $MainData["ToCreatedDate"]}}@endif' required>
											</div>

											<input type="hidden" id="txtRecId" name="txtRecId"/>
										</div>
									</div>
								</div>
								<!--
							...
							-->
							<div class="col-lg-6 col-md-4 col-sm-12 col-xs-12" style="margin:20px 0;">

								<button type="submit" class="btn btn-info" name="BtnSearch"><i class="fa fa-search mr7"></i>Search</button>&nbsp;

								<button type="submit" class="btn btn-info" name="BtnBackup" id="BtnBackup">Backup</button>&nbsp;

								<button type="submit" class="btn btn-info" name="BtnBackupArchive" id="BtnBackupArchive">Backup & Archive</button>&nbsp;
				...
				<table id="datatable_fixed_column" class="table table-striped table-bordered table-hover" width="100%">
					<thead>
						...
						<tr>
							...
							<th>
								<i class="fa fa-fw fa-user text-muted hidden-md hidden-sm hidden-xs"></i> FormID
							</th>
						...
						<tr>
							<td>
								<div class="checkbox pull-left">
									<label>
										<input type="checkbox" name="RecId" value="{{ $caldata->RecId }}" class="checkbox style-1 userListCheck">
										<span></span>
									</label>
								</div>
...
$('#datatable_fixed_column').on("change", ".userListCheck", function (event) {
		...
		$("#BtnBackup").click(function () {    
			if($('.userListCheck:checkbox:checked').length == 0) {
				alert('Please select at least one record to backup.');
				return false;
			}
		});

		$("#BtnBackupArchive").click(function () {    
			if($('.userListCheck:checkbox:checked').length == 0) {
				alert('Please select at least one record to backup & archive.');
				return false;
			}
		});

		$("#BtnRestore").click(function () {  
			$('#FromCreatedDate').removeAttr('required');
			$('#ToCreatedDate').removeAttr('required');
		});

		$('#userListCheck').on('change',function(){
      selectedMenuGroup = $('#group_menu').val();
      $('#selectedMenuGroup').val(selectedMenuGroup);
      val= document.getElementById('userListCheck').checked;
      var oTable = $('.cell-border').DataTable();

      $('input', oTable.cells().nodes()).prop('checked',false);

      if(val){
        $('.userListCheck').prop('checked', true);
        $('input', oTable.cells().page.info()).prop('checked',true);
