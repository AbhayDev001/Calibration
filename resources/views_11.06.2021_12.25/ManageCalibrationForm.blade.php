		...
		<div class="row">
			<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				<div class="product-content product-wrap clearfix product-deatil">
					<form id="SaveUser" name="frm" method="post" action="{{ url('SaveCaliForm') }}">
						<input type="hidden" value="{{csrf_token()}}" name="_token" />
						<div class="col-lg-12">
							<div class="row top-menu-box">
								<b>Add Calibration Form</b>
						...
						<div class="col-lg-12">
							<div class="row mt20">
								<fieldset class="col-lg-5 mobile-mt20">
									<div class="row">
										<label class="col-md-6 control-label form-lable"><b>Form Name:</b></label>
								...
								<fieldset class="col-lg-5 mobile-mt20">
									<div class="row">
										<label class="col-md-6 control-label form-lable"><b>Calibration Form Type:</b></label>
										<div class="col-md-6 input-area">
											<select type="text" class="form-input form-control CType" name="CType">
												@php
						...
						<table id="datatable_fixed_column" class="table table-striped table-bordered table-hover" width="100%">
							<thead>
								<tr>
									...
									<th>
										<input type="text" class="form-control FillterData" placeholder="Calibration Form Type" style="width: 100%;">
									</th>
									<th>
								...
								<tr>
									...
									<th><i class="fa fa-fw fa-sort-numeric-asc text-muted hidden-md hidden-sm hidden-xs"></i> Calibration Form Type</th>
							...
							<tbody>
								@php
								$FormTypeMaster=DB::table('calibrationform')->orderBy('CreatedDate', 'desc')->get();
								@endphp
								...
								<tr>
									...
									<td>
										@php
										$GetCalFormType=DB::table('calibrationformtype')->where('RecId',$formtype->CType)->get();
										echo $GetCalFormType[0]->Name;
										@endphp
									</td>
									<td>

									<?php $CreatedByName=DB::table('usermaster')->where('UserId',$formtype->CreatedBy)->first(); 
									...
									<td>
										<a href="javascript:void(0)" data-RecId="{{ $formtype->RecId }}" class="btn btn-xs btn-default btnEditForm"><i class="fa fa-pencil"></i></a>
										<form name="frm" method="post" action="{{ url('ActiveDeactiveCalForm') }}" style="display: inline-block;">
											<input type="hidden" value="{{csrf_token()}}" name="_token" />
		...
		<div class="modal fade" id="EditFormType" role="dialog">
			<div class="modal-dialog">
				<div class="modal-content">
					<form id="UpdUser" name="frm" method="post" action="{{ url('UpdCaliForm') }}">
						<input type="hidden" value="{{csrf_token()}}" name="_token" />
						<input type="hidden" value="0" name="RecId" class="UpdRecId" />
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal">&times;</button>
							<h4 class="modal-title">Edit Calibration Form</h4>
						</div>
						<div class="modal-body">
							<div class="input-area">
								<div class="form-group">
									<label for="FormTypeName">Form Name</label>
									<input type="text" class="form-control UpdFormTypeName" id="FormTypeName" name="FormTypeName" autocomplete="off">
								</div>
							</div>
							<div class="input-area">
								<div class="form-group">
									<label for="FormDays">Calibration Form Type</label>
									<select type="text" class="form-control UpdCType" name="CType">
										@php
										$CalibrationFormType=DB::table('calibrationformtype')->where('IsActive','1')->get();
										@endphp
										@foreach($CalibrationFormType as $FormType)
...
<script type="text/javascript">
	$(".btnSaveForm").click(function(){
		...
		if($(".CType").val()=='')
		{
			$(".CType").parent("div").children(".tooltip").remove();
			$(".CType").parent("div").append("<b class='tooltip tooltip-bottom-right'>Required field</b>",removetooltip('CType'));
			$(".CType").focus();
			return false;
		}
	});
	...
	$(".btnUpdForm").click(function(){
		...
		if($(".UpdCType").val()=='')
		{
			$(".UpdCType").parent("div").children(".tooltip").remove();
			$(".UpdCType").parent("div").append("<b class='tooltip tooltip-bottom-right' style='opacity:1;'>Required field</b>",removetooltip('UpdCType'));
			$(".UpdCType").focus();
			return false;
		}
	});
...
<script type="text/javascript">
	$(".btnEditForm").click(function(){
		$.get('AjaxGetCaliForm/'+ $(this).attr("data-RecId"),function(data){
			if(data!='')
			{
				$(".UpdRecId").val(data['RecId']);
				$(".UpdFormTypeName").val(data['Name']);
				$(".UpdCType").val(data['CType']);
