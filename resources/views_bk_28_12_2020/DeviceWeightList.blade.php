							...
							<table id="datatable_fixed_column" class="table table-striped table-bordered table-hover" width="100%">
								<thead> 
									<tr>
										...
										<th>
											<input type="text" class="form-control FillterData" placeholder="Calibration Type" style="width: 100%;">
										</th>
										<th>
									...
									<tr>
										...
										<th><i class="fa fa-fw fa-wrench text-muted hidden-md hidden-sm hidden-xs"></i> Calibration Type</th>
								...
								<tbody>
									@php
									$DeviceWeight=DB::table('deviceweight')->groupBy("InstrumentId","DeviceId","CalibrationTypeId")->orderBy('CreatedDate', 'desc')->select("InstrumentId","DeviceId","CalibrationTypeId")->get();
									@endphp
									@foreach($DeviceWeight as $Dweight)
									<tr>
										<td>
											@php
											$InstrumentName=DB::table('instrumentmaster')->where('RecId',$Dweight->InstrumentId)->first();
											echo $InstrumentName->Name;
											@endphp
										</td>
										<td>
											@php
											$DeviceName=DB::table('devicemaster')->where('RecId',$Dweight->DeviceId)->first();
											echo $DeviceName->Name;
											@endphp
										</td>
										<td>
											@php
											$CalibrationTypeIdName=DB::table('calibrationformtype')->where('RecId',$Dweight->CalibrationTypeId)->first();
											echo $CalibrationTypeIdName->Name;
											@endphp
										</td>
										<td>
											@php
											$CreatedUserId=DB::table('deviceweight')->where('InstrumentId',$Dweight->InstrumentId)->where('DeviceId',$Dweight->DeviceId)->where('CalibrationTypeId',$Dweight->CalibrationTypeId)->select("CreatedBy")->first();

											$CreatedByName=DB::table('usermaster')->where('UserId',$CreatedUserId->CreatedBy)->first();
											echo $CreatedByName->UserName;
											@endphp
										</td>
										<td>
											<form name="frm" method="POST" action="{{ url('EditDeviceWeight') }}" style="display: inline-block;"> 
												<input type="hidden" value="{{csrf_token()}}" name="_token" />
												<input type="hidden" value="{{ $Dweight->InstrumentId }}" name="InstrumentId" />
												<input type="hidden" value="{{ $Dweight->DeviceId }}" name="DeviceId" />
												<input type="hidden" value="{{ $Dweight->CalibrationTypeId }}" name="CalibrationTypeId" />
												<button type="submit" title="Edit" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></button>
											</form>
											<form name="frm" method="POST" action="{{ url('DeleteDeviceWeight') }}" style="display: inline-block;"> 
												<input type="hidden" value="{{csrf_token()}}" name="_token" />
												<input type="hidden" value="{{ $Dweight->InstrumentId }}" name="InstrumentId" />
												<input type="hidden" value="{{ $Dweight->DeviceId }}" name="DeviceId" />
												<input type="hidden" value="{{ $Dweight->CalibrationTypeId }}" name="CalibrationTypeId" />
