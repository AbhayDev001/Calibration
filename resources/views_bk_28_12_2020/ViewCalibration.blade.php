					...
					<div class="col-lg-12">
						<div class="row top-menu-box">
							<?php /*<ul class="top-box-menu-bar">
								...
								<li>
									<form name="frm" method="post" action="{{ url('VerifyCalibration') }}">
										<input type="hidden" value="{{csrf_token()}}" name="_token" />
										<input type="hidden" name="Status" value="20" />
										<input type="hidden" name="RecId" value="{{ $MainData['CalData'][0]['RecId'] }}" />
										<button type="submit"><i class="fa fa-lg fa-fw fa-check"></i> <span class="area-text">Verify</span></button>
									</form>
								</li>
								<li>
									<form name="frm" method="post" action="{{ url('VerifyCalibration') }}">
										...
										<input type="hidden" name="RecId" value="{{ $MainData['CalData'][0]['RecId'] }}" />
										<button type="submit"><i class="fa fa-lg fa-fw fa-close"></i> <span class="area-text">Decline</span></button>
									</form>
								</li>
								@endif
								@if(Session::get('LoginData')['Role']==4 && ($MainData['CalData'][0]->Status==20 || $MainData['CalData'][0]->Status==40))
								<li>
									<form name="frm" method="post" action="{{ url('ApproveCalibration') }}">
										<input type="hidden" value="{{csrf_token()}}" name="_token" />
										<input type="hidden" name="RecId" value="{{ $MainData['CalData'][0]['RecId'] }}" />
										<input type="hidden" name="Status" value="30" />
										<button type="submit"><i class="fa fa-lg fa-fw fa-check"></i> <span class="area-text">Approve</span></button>
									</form>
								</li>
								@endif
								...
								<ul class="top-box-menu-bar">
									<li>
										<a href="{{ url('/PrintCalibration/'.$MainData['CID']) }}" target="_blank" class="btn btn-xs btn-default"><i class="fa fa-print"></i> Print Calibration Details</a>
									</li>
								</ul>
									...
									<div class="row">
										...
										<div class="col-md-6 input-area">
											...
											<div class="inline-group BalanceChecked">
												<label class="radio">
													<input type="radio" class="userroles" name="BalanceChecked" value="1" checked="checked" disabled="disabled">
													<i></i>Yes
												</label>
												<label class="radio">
													<input type="radio" class="userroles" name="BalanceChecked" value="2" disabled="disabled">
													<i></i>No
												</label>
											</div>
											...
											<div class="inline-group BalanceChecked">
												<label class="radio">
													<input type="radio" class="userroles" name="BalanceChecked" value="1"  disabled="disabled">
													<i></i>Yes
												</label>
												<label class="radio">
													<input type="radio" class="userroles" name="BalanceChecked" value="2" checked="checked" disabled="disabled">
													<i></i>No
												</label>
											</div>
											@else
									...
									<div class="row">
										...
										<div class="col-md-8 input-area">
											...
											<div class="inline-group InternalCalibration">
												<label class="radio">
													<input type="radio" class="userroles" name="InternalCalibration" value="1" checked="checked" disabled="disabled">
													<i></i>Passes
												</label>
												<label class="radio">
													<input type="radio" class="userroles" name="InternalCalibration" value="2" disabled="disabled">
													<i></i>Fails
												</label>
											</div>
											...
											<div class="inline-group InternalCalibration">
												<label class="radio">
													<input type="radio" class="userroles" name="InternalCalibration" value="1" disabled="disabled">
													<i></i>Passes
												</label>
												<label class="radio">
													<input type="radio" class="userroles" name="InternalCalibration" value="2" checked="checked" disabled="disabled">
													<i></i>Fails
												</label>
											</div>
											@else
		...
		$(document).ready(function (){
					...
					if(DeviceFilePath!='')
					{
						$.get('../AjaxGetFileData/'+ DeviceFilePath,function(data){
							if(data['mg']>0 && data['mg']!="")
							{
								...
								var comment=$("#CommentModel").find(".LineComment").val();
								...
								var Data='{"Comments":"'+comment+'","CalibrationLineRecId":"'+LineRecId+'","LineId":"'+LineId+'","StWeight":"'+StWeight+'","CertWeight":"'+CertWeight+'","DispWeight":"'+DispWeight+'","DifferentAB":"'+DifferentAB+'","AccpWeight":"'+AccpWeight+'","Result":"'+Result+'","Tfile":"'+Tfile+'","Ifile":"'+Ifile+'","CalibrationStatus":"'+$(".CalibrationStatus").val()+'","RefRecId":"'+$(".FormRecId").val()+'"}';
			...
			$(".btnAddComment1").click(function(){
				if($(".txtComment").val()!='')
				{
					...
					var Data1='{"Comments":"'+comment+'","CalibrationLineRecId":"'+LineRecId+'","CalibrationStatus":"'+$(".CalibrationStatus").val()+'","RefRecId":"'+$(".FormRecId").val()+'","Type":"1"}';

					$.get('../AjaxSaveComment/'+Data1,function(data){
						if(data!=100)
						{
							alert("Unable to save data");
							LoadCommentTable();
